...
}`),e.fragment.code.add(a.glsl`vec4 premultipliedColor(vec3 rgb, float alpha) {
return vec4(rgb * alpha, alpha);
}`),e.fragment.code.add(a.glsl`vec4 laserlineProfile(float dist) {
if (dist > glowWidth) {
return vec4(0.0);
}
...
}`),e.fragment.code.add(a.glsl`bool laserlineReconstructFromDepth(out vec3 pos, out vec3 normal, out float depthDiscontinuityAlpha) {
...
return true;
}`),l.contrastControlEnabled?e.fragment.code.add(a.glsl`float rgbToLuminance(vec3 color) {
...
return color * alpha;
}`):e.fragment.code.add(a.glsl`vec4 laserlineOutput(vec4 color) {

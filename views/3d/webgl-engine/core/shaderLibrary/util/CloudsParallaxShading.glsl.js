...
*/
define(["exports","../shading/PiUtils.glsl","../../shaderModules/interfaces"],(function(o,t,a){"use strict";function e(o){o.fragment.uniforms.add("anchorPosition","vec3").add("anchorPositionCrossFade","vec3").add("fadeInOutFactor","float").add("cloudsHeight","float").add("rotationMatrixClouds","mat4").add("rotationMatrixCloudsCrossFade","mat4").add("radiusCurvatureCorrectionFactor","float").add("radius","float").add("cameraPosition","vec3").add("totalFadeInOut","float").add("crossFade","int").add("crossFadeAnchorFactor","float").add("cloudVariables","vec2"),o.include(t.PiUtils),o.fragment.code.add(a.glsl`vec3 intersectWithCloudLayer(vec3 dir, vec3 camPos, vec3 spherePos, float radiusReduction)
{
...
}`),o.fragment.code.add(a.glsl`vec3 correctForPlanetCurvature(vec3 dir)
{
dir.z = dir.z*(1.-radiusCurvatureCorrectionFactor) + radiusCurvatureCorrectionFactor;
return dir;
}`),o.fragment.code.add(a.glsl`vec3 rotateDirectionToAnchorPoint(mat4 rotMat, vec3 inVec)
{
return (rotMat * vec4(inVec, 0.0)).xyz;
}`),o.fragment.code.add(a.glsl`const float SUNSET_TRANSITION_FACTOR = 0.3;
...
}`),o.fragment.code.add(a.glsl`vec4 getCloudData(vec3 rayDir)
{
...
}`),o.fragment.code.add(a.glsl`vec4 renderCloud(vec3 worldRay)
{
...
}`),o.fragment.code.add(a.glsl`vec4 renderCloudCrossFade(vec3 worldRay)
{
vec3 intersectionPoint = intersectWithCloudLayer(normalize(worldRay), cameraPosition, anchorPosition, 0.0);
vec3 worldRayRotated = rotateDirectionToAnchorPoint(rotationMatrixClouds, normalize(intersectionPoint));
vec3 worldRayRotatedCorrected = correctForPlanetCurvature(worldRayRotated);
...
}`)}o.CloudsParallaxShading=e,Object.defineProperty(o,"__esModule",{value:!0})}));

...
*/
define(["exports","../../../core/shaderModules/interfaces","../../../core/shaderLibrary/util/DoublePrecision.glsl"],(function(o,e,t){"use strict";function n(o,n){const r=o.vertex;r.uniforms.add("uDistanceFalloffFactor","float"),r.code.add(e.glsl`
    float distanceBasedPerspectiveFactor(float distance) {
      return clamp(sqrt(uDistanceFalloffFactor / distance), 0.0, 1.0);
    }
  `),r.uniforms.add("uComponentDataTex","sampler2D"),r.uniforms.add("uComponentDataTexInvDim","vec2"),o.attributes.add("componentIndex","float"),r.constants.add("componentColorFieldOffset","float",0),r.constants.add("componentOtherFieldOffset","float",1),r.constants.add("componentFieldCount","float",2),r.constants.add("lineWidthFractionFactor","float",8),r.constants.add("extensionLengthOffset","float",128),r.constants.add("componentTexWidth","float",4096),r.code.add(e.glsl`
  ...
  `),n.legacy?r.code.add(e.glsl`
    ...
    `):(r.uniforms.add("uTransformNormal_GlobalFromModel ","mat3"),r.code.add(e.glsl`
      vec3 _modelToWorldNormal(vec3 normal) {
        return uTransformNormal_GlobalFromModel * normal;
      }
    `)),n.silhouette?(o.attributes.add("normalA","vec3"),o.attributes.add("normalB","vec3"),r.code.add(e.glsl`
      vec3 worldNormal() {
        return _modelToWorldNormal(normalize(normalA + normalB));
      }
    `)):(o.attributes.add("normal","vec3"),r.code.add(e.glsl`
      vec3 worldNormal() {
    ...
    `):(o.vertex.include(t.DoublePrecision,n),r.code.add(e.glsl`
      vec3 worldFromModelPosition(vec3 position) {

 ...
 *
 * [![threshold mode](../assets/img/apiref/widgets/shadow-cast/shadow-cast-threshold.png)](../sample-code/widgets-shadow-cast/)
 *
 ...
 *
 * [![duration mode](../assets/img/apiref/widgets/shadow-cast/shadow-cast-duration.png)](../sample-code/widgets-shadow-cast/)
 *
 ...
 *
 * [![discrete mode](../assets/img/apiref/widgets/shadow-cast/shadow-cast-discrete.png)](../sample-code/widgets-shadow-cast/)
 *
 * The defaults for the time range and visualization settings can be changed using the
 * {@link module:esri/widgets/ShadowCast/ShadowCastViewModel}.
 *
 ...
 *
 * @module esri/widgets/ShadowCast
 * @since 4.21
 *
 * @see [ShadowCast.tsx (widget view) [deprecated since 4.21]]({{ JSAPI_ARCGIS_JS_API_URL }}/widgets/ShadowCast.tsx)
 * @see [ShadowCast.scss]({{ JSAPI_ARCGIS_JS_API_URL }}/themes/base/widgets/_ShadowCast.scss)
 * @see module:esri/widgets/ShadowCast/ShadowCastViewModel
 * @see {@link module:esri/views/SceneView#environment SceneView.environment}
 * @see module:esri/views/ui/DefaultUI
 * @see [Sample - Shadow cast](../sample-code/widgets-shadow-cast/)
 */
...
import { CSS } from "esri/widgets/ShadowCast/css";
import { ShadowCastState } from "esri/widgets/ShadowCast/ShadowCastState";
import ShadowCastViewModel from "esri/widgets/ShadowCast/ShadowCastViewModel";
import ShadowCastVisibleElements from "esri/widgets/ShadowCast/ShadowCastVisibleElements";
import { ShadowVisualizationType } from "esri/widgets/ShadowCast/ShadowVisualizationType";

// esri.widgets.ShadowCast.components
import { DiscreteConfigurator } from "esri/widgets/ShadowCast/components/DiscreteConfigurator";
import { DurationConfigurator } from "esri/widgets/ShadowCast/components/DurationConfigurator";
import { ShadowTooltip } from "esri/widgets/ShadowCast/components/ShadowTooltip";
import { ThresholdConfigurator } from "esri/widgets/ShadowCast/components/ThresholdConfigurator";
import { TimezonePicker } from "esri/widgets/ShadowCast/components/TimezonePicker";

// esri.widgets.ShadowCast.t9n
import ShadowCastMessages from "esri/widgets/ShadowCast/t9n/ShadowCast";
...
interface OptionalConstructProperties {
  ...
  visibleElements: Partial<ShadowCastVisibleElements>;
  headingLevel: HeadingLevel;
}

type ConstructProperties = Partial<OptionalConstructProperties> &
  WidgetProperties<Castable<ShadowCastViewModel>>;
  ...
  tickConfigs: [
    {
      ...
      tickCreatedFunction: (
        ...
        if (match) {
          labelElement.innerHTML = `${match[1]}<br><div class="${CSS.timeRangeAMPMLabel}">${match[2]}</div>`;
        }
      }
    },
    {
...
@subclass("esri.widgets.ShadowCast")
class ShadowCast extends Widget implements ConstructProperties {
  //----------------------------------------------------------------------------
  //
  //  Lifecycle
  //
  ...
  /**
   * @constructor
   * @alias module:esri/widgets/ShadowCast
  ...
  constructor(properties?: ConstructProperties, parentNode?: string | Element) {
    ...
    if (!properties?.viewModel) {
      this._defaultViewModel = new ShadowCastViewModel({ view: properties?.view });
  ...
  protected override loadDependencies(): Promise<any> {
    return import("@esri/calcite-components/dist/custom-elements/bundles/select");
  }

  override render(): VNode {
    const { visibleElements, viewModel } = this;
    const disabled = viewModel.state === ShadowCastState.Disabled;
   ...
   * that controls this widget's behavior. See the {@link module:esri/widgets/ShadowCast/ShadowCastViewModel}
   * class to access all properties and methods on the widget.
   *
   * @name viewModel
   * @instance
   * @type {module:esri/widgets/ShadowCast/ShadowCastViewModel}
   * @autocast
   */
  @property()
  override viewModel: ShadowCastViewModel = null;
   ...
   *
   * @typedef module:esri/widgets/ShadowCast~VisibleElements
   *
   ...
   * @type {module:esri/widgets/ShadowCast~VisibleElements}
   * @example
   ...
   */
  @property({
    type: ShadowCastVisibleElements,
    nonNullable: true
  })
  visibleElements: Partial<ShadowCastVisibleElements> = new ShadowCastVisibleElements();
  ...
  @property()
  private _defaultViewModel: Maybe<ShadowCastViewModel> = null;

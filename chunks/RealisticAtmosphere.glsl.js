...
*/
define(["exports","../views/3d/webgl-engine/core/shaderLibrary/util/ColorConversion.glsl","../views/3d/webgl-engine/core/shaderModules/interfaces","../views/3d/webgl-engine/core/shaderModules/ShaderBuilder"],(function(e,a,t,r){"use strict";function n(e){const n=new r.ShaderBuilder;return n.attributes.add("position","vec2"),n.attributes.add("uv0","vec2"),n.varyings.add("worldRay","vec3"),n.varyings.add("vtc","vec2"),e.haze&&n.varyings.add("eyeDir","vec3"),n.vertex.uniforms.add("projectionInverse","mat4"),n.vertex.uniforms.add("viewInverse","mat4"),n.vertex.code.add(t.glsl`
    void main(void) {
...
#define oneOverScaleDepth vec2(atmosphereParameters1.w, atmosphereParameters2.w)`),e.haze||n.fragment.code.add(t.glsl`#define g atmosphereParameters2.x
#define gSq atmosphereParameters3.x
#define miePhaseCoefficients atmosphereParameters3.y
#define lowerAlphaBlendBound atmosphereParameters3.z`),n.fragment.code.add(t.glsl`
  ...
  `),e.haze||n.fragment.code.add(t.glsl`float surfaceBlend = 0.0;
...
surfaceBlend = smoothstep(0.0, 1.0, relDist * relDist);
  ...
  `),e.haze&&n.fragment.code.add(t.glsl`vec3 end = cameraPosition + worldSpaceRay * rayEnd;
...
}`),n.fragment.code.add(t.glsl`vec2 opticalStartDepth = exp(scatterDistance * oneOverScaleDepth);
...
vec3 samplePoint = start + sampleRay * 0.5;`),e.haze?n.fragment.code.add(t.glsl`float cameraAngle = dot(-worldSpaceRay, end) / length(end);
...
vec2 scaledValuesDepth = scaledValues * shellDepth;`):n.fragment.code.add(t.glsl`float cameraAngle = dot(worldSpaceRay, start / length(start));
float angleMultiplier = cameraAngle > 0.0 ? cameraAngle : 0.0;
float scaleCameraAngle = scale(cameraAngle);
vec2 cameraOffset = scaleCameraAngle * opticalStartDepth * shellDepth;`),n.fragment.code.add(t.glsl`vec3 frontColor = vec3(0.0);
...
vec2 depth = exp(-altitude * scaleOverScaleDepth);`),e.haze?n.fragment.code.add(t.glsl`vec2 scatter = depth * scaledValuesDepth - cameraOffset;`):n.fragment.code.add(t.glsl`float lightAngle = dot(lightingMainDirection, samplePoint) / height;
float cameraAngle = dot(worldSpaceRay, samplePoint) / height;
float tmpScaledValues = scale(lightAngle) - scale(cameraAngle);
vec2 scatter = cameraOffset + tmpScaledValues * depth * shellDepth;`),n.fragment.code.add(t.glsl`attenuate = exp(-scatter.x * invWavelengthScaled);
...
float LdotRSq = LdotR * LdotR + 1.0;`),e.haze?n.fragment.code.add(t.glsl`vec3 colorCoefficients = (scaledLength * 0.75 * LdotRSq) * (krESun * invWavelength + kmESun );
vec3 color = colorCoefficients * frontColor;
vec3 colorBlue = colorCoefficients * frontColorBlue;`):n.fragment.code.add(t.glsl`vec3 rayleighCoefficients = (scaledLength * 0.75 * LdotRSq * krESun) * invWavelength;
...
vec3 colorBlue = rayleighCoefficients * frontColorBlue + mieCoefficients * frontColorBlue;`),n.fragment.code.add(t.glsl`vec3 ldrBlue = expTM(colorBlue, 2.0 * exposure);
vec3 ldrRed = expTM(color, exposure);
vec3 LDR = mix(ldrBlue, ldrRed, 0.2);`),e.haze?n.fragment.code.add(t.glsl`LDR *= (1.0 - cameraAngle);
vec3 hsv = rgb2hsv(LDR);
hsv.y = clamp(hsv.y * 1.5, 0.0, 1.0);
LDR = hsv2rgb(hsv);
vec3 finalColor = LDR;`):n.fragment.code.add(t.glsl`vec3 ldrReinhard = reinhardTM(color, exposure);
...
hsv.y = clamp(hsv.y * 1.5, 0.0, 1.0);
finalColor = hsv2rgb(hsv);`),n.fragment.code.add(t.glsl`
